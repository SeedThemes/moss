...
if ( ! function_exists( 'moss_posted_on' ) ) :
	...
	function moss_posted_on() {
		...
		$posted_on = sprintf(
			...
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark"><i data-feather="calendar"></i>' . $time_string . '</a>'
		);
...
if ( ! function_exists( 'moss_posted_by' ) ) :
	...
	function moss_posted_by() {
		$byline = sprintf(
			...
			'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '"><i data-feather="user"></i>' . esc_html( get_the_author() ) . '</a></span>'
		);
